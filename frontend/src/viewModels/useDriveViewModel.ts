...
export const useDriveViewModel = (userId: string, appId: string) => {
...
const loadMetadata = async () => {
  ...
  for (let field of metaFields) {
    if (field.foreign) {
      ...
      if (!fetchedForeign.has(foreignName)) {
        ...
        const data = await driveService.getForeignResource(foreignName);
  ...
  for (let field of metaFields) {
    if (field.isEnum) {
      ...
      if (!fetchedEnum.has(enumName)) {
        ...
        const values = await driveService.getEnum(enumName);
  ...
  const save = async (accessToken: string) => {
    ...
    const response = await driveService.create({
      accessToken,
      params,
    });
