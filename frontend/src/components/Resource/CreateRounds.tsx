...
import styles from "../Styles/CreateCertificate.module.css"; // ✅ YOUR UI
...
function getAccessToken(): string | null {
  return Cookies.get("access_token") || getCookie("access_token") || null;
}
...
const CreateRounds = () => {
  ...
  useQuery({
    queryKey: ["resMetaData", "roundsCreate"],
    queryFn: async () => {
      const res = await fetch(apiConfig.getResourceMetaDataUrl("Rounds"));
      const data = await res.json();

      setFields(data?.[0]?.fieldValues || []);

      for (const f of data?.[0]?.fieldValues || []) {
        if (f.foreign && !fetchedResources.current.has(f.foreign)) {
          fetchedResources.current.add(f.foreign);
          const d = await fetchForeignResource(f.foreign);
          setForeignKeyData((p: any) => ({ ...p, [f.foreign]: d }));
        }

        if (f.isEnum && !fetchedEnum.current.has(f.possible_value)) {
          fetchedEnum.current.add(f.possible_value);
          const e = await fetchEnum(f.possible_value);
          setEnums((p: any) => ({ ...p, [f.possible_value]: e }));
        }
      }
      return data;
    },
  ...
  // ---------------- Drive Options ----------------
  const { data: driveOptions } = useQuery({
    queryKey: ["driveOptions", "roundsCreate"],
    queryFn: async () => {
      const token = getAccessToken();
  ...
  const drives = useMemo(
    () => (Array.isArray(driveOptions) ? driveOptions.filter((d) => d.archived !== "Y") : []),
    [driveOptions]
  );
  ...
  const handleCreate = async () => {
    try {
      const token = getAccessToken();
      const err = validateStartBeforeEnd(dataToSave?.startdate, dataToSave?.enddate);
      if (err) return setToast({ type: "error", msg: err });
    ...
    <div className={styles.batchformCard}>
      <div className={styles.certificateFormWrapper}>
        ...
        <div className={styles.formGrid}>
          ...
          <div className={styles.formGroup}>
            <label className={styles.formLabel}>
              <span className={styles.required}>*</span> Drive
            </label>
            <select
              className={styles.formControl}
              value={dataToSave.drive_id || ""}
        ...
        <div className={styles.buttonRow}>
          <button className={styles.primaryBtn} onClick={handleCreate} disabled={!!dateError}>
            Submit
          </button>
        </div>
      </div>
