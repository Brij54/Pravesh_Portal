...
import DriveModel from '../../models/DriveModel';
  ...
  const {data:dataRes,isLoading:isLoadingDataRes,error:errorDataRes}= useQuery({
    queryKey: ['resourceData', 'driveRead'],
     queryFn: async () => {
...
const accessToken = getCookie("access_token");
  ...
  useEffect(() => {
    if (dataRes) {
      ...
      const modelObjects = dataRes.map((obj: any) => DriveModel.fromJson(obj));
    ...
    console.log("data initialized in store",getUserAllData(getUserIdFromJWT()))
  }, [dataRes]);

  const {data: dataResMeta,isLoading:isLoadingDataResMeta,error:errorDataResMeta} = useQuery({
    queryKey: ['resourceMetaData', 'driveRead'],
   queryFn: async () => {
      const response = await fetch(
        `${apiConfig.getResourceMetaDataUrl('drive')}?`,
        {
          method: "GET",
